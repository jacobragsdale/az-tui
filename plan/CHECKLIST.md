...
2. `az-tui setup`: every cluster is listed, credentials written, and the
...
3. Trim the printed `[[clusters]]` blocks to the namespaces you want tabs
   for and put them in `~/.config/az-tui/config.toml` (or run `setup
   --write` and edit).
4. `az-tui doctor`: every scope `ok`, each under a second. A `forbidden`
   here is RBAC, not the tool; a `FAIL` names what to fix.
5. `az-tui --no-cache`: the first frame inside a second, each tab's badge
...
13. `s`: a secret's keys and sizes; `v` reveals one for sixty seconds and it
    ...
    `grep -rc 'the value' ~/.local/share/az-tui/` is 0 for every file.
